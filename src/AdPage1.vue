  ...
  <div class="ad-page-container">
    ...
    <div v-else class="content-wrapper">
      ...
      <div class="security-notice">
        <p>🔒 Secure Download | ⏱️ Link expires in 60 minutes</p>
      </div>
...
if (wasReload === 'true') {
  ...
  // Alternatively, try external script
  // this.loadExternalAdScript();
  ...
  // Auto-trigger on user interaction (click anywhere)
  document.addEventListener('click', () => {
    console.log('👆 Click detected - checking for pop trigger');
  ...
  loadHilTopAd() {
    try {
      console.log('🔧 Loading Hiltop Ad Script...');
  ...
  // Alternative method - Load ad script from external URL
  loadExternalAdScript() {
    try {
      console.log('🔧 Loading External Ad Script...');
      
      const script = document.createElement("script");
      script.type = "text/javascript";
      script.async = true;
      script.src = "//impassioned-beat.com/c/D.9Z6vbq2P5DlISLW_Qw9gNojycmyJMVTzInyDMkSS0P2lNVzfIMx_MijsI/yu";
      
      script.onerror = () => {
        console.error('❌ Failed to load external ad script');
      };
      
      script.onload = () => {
        console.log('✅ External ad script loaded successfully');
      };
  ...
  // Method to trigger pop-under manually (for testing)
  triggerPopUnder() {
    console.log('🎯 Attempting to trigger pop-under...');
    
    // Check if pop-under functions exist
    if (window.pop1stp_pop) {
      console.log('✅ Pop1stp found');
      try {
        window.pop1stp_pop.doPop(window.pop1stp_pop.getPopUrl());
      } catch (e) {
        console.error('❌ Error triggering pop:', e);
      }
    } else {
      console.warn('⚠️ Pop1stp not found - ad script may not be loaded');
    }
  },

  // Check ad script status
  checkAdStatus() {
    console.log('🔍 Checking ad script status...');
    console.log('Pop1stp available:', !!window.pop1stp_pop);
    console.log('SRC Pop available:', !!window.src_pop);
    
    // Check for ad containers
    const containers = [
      'container-banner-top',
      'lower-ad-holder',
      'container-d3e5f275b234c1684bbd1dd364aced62'
    ];
